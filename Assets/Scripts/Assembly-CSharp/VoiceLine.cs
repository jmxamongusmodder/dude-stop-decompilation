...
public class VoiceLine
{
	...
	public VoiceLine(StandaloneLevelVoiceGuid entry)
	{
		...
		if (this.info.fmodName.Contains("event:/"))
		{
			...
			if (VoiceLine.cache01 == null)
			{
				VoiceLine.cache01 = new EVENT_CALLBACK(VoiceLine.markerFound);
			}
			this.voice.setCallback(VoiceLine.cache01, EVENT_CALLBACK_TYPE.STOPPED | EVENT_CALLBACK_TYPE.TIMELINE_MARKER);
		}
		else
		{
			if (VoiceLine.cache1 == null)
			{
				VoiceLine.cache1 = new EVENT_CALLBACK(VoiceLine.markerFound);
			}
			this.voice.setCallback(VoiceLine.cache1, EVENT_CALLBACK_TYPE.STOPPED | EVENT_CALLBACK_TYPE.CREATE_PROGRAMMER_SOUND | EVENT_CALLBACK_TYPE.DESTROY_PROGRAMMER_SOUND | EVENT_CALLBACK_TYPE.TIMELINE_MARKER);
		}
	}
	...
	[CompilerGenerated]
	private static EVENT_CALLBACK cache01;

	// Token: 0x0400105D RID: 4189
	[CompilerGenerated]
	private static EVENT_CALLBACK cache1;
}
